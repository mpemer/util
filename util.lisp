...
(define-test parse-number?-test
  ...
  (parachute:is = (parse-number? "123.45") 2469/20
                "Parsing a valid decimal number should return its rational representation.")
...
(define-test insert-hash-test
  ...
  (let ((ht (make-hash-table)))
    ...
    (multiple-value-bind (value exists) (gethash :key1 ht)
      (parachute:is equal value '(:new-value . :original)
                    "Inserting with an existing key should update the value at that key.")
  ...
  (let ((ht (make-hash-table)))
    ...
    (multiple-value-bind (value exists) (gethash :key2 ht)
      (parachute:is equal value '(:value2) "Inserting with a non-existent key should create a new key-value pair.")
      (parachute:true exists "The key should exist in the hash table.")))
  )
...
(define-test roundto-test
  ...
  (parachute:is = (roundto 123.456 0) 123
                "Rounding to 0 decimal places should round to the nearest integer.")

  ;; Test rounding to 1 decimal place
  (parachute:is = (roundto 123.456 1) 247/2
                "Rounding to 1 decimal place should round to the nearest tenth.")

  ;; Test rounding to 2 decimal places
  (parachute:is = (roundto 123.456 2) 6173/50
                "Rounding to 2 decimal places should round to the nearest hundredth.")

  ;; Test rounding to a negative number of decimal places
  (parachute:is = (roundto 123.456 -1) 120
                "Rounding to a negative number of decimal places should round to tens, hundreds, etc.")

  ;; Test with an integer input
  (parachute:is = (roundto 123 2) 123
                "Rounding an integer should work correctly, adding decimal zeroes as necessary.")
...
(define-test process-list-test
  ...
  (let ((chain (list #'identity #'1+ #'1+))) ; Increment twice
    (parachute:is equal (process-list '(1 2 3) :chain chain :monads #'identity) '(3 4 5)
                   "Applying a simple chain should increment each element twice."))
  ...
  (let ((chain (list #'1+))
    ...
    (parachute:is equal (process-list '(1 2 3) :chain chain :monads monads) '(4 6 8)
                   "Applying both chain and monads should increment and then double each element."))

  ;; Test with empty list
  (parachute:is eq (process-list '() :chain (list #'1+) :monads (list (lambda (x) (* x 2)))) '()
...
(define-test splice-test
  ...
  (parachute:is equal (splice '(1 2 3) '(4 5 6)) '(3 2 1 4 5 6)
                 "Splicing '(1 2 3) to '(4 5 6) should yield '(3 2 1 4 5 6).")

  ;; Test with an empty ADD list
  (parachute:is equal (splice '() '(1 2 3)) '(1 2 3)
                 "Splicing an empty list to '(1 2 3) should yield '(1 2 3).")

  ;; Test with an empty LST list
  (parachute:is equal (splice '(1 2 3) nil) '(3 2 1)
                 "Splicing '(1 2 3) to an empty list should yield '(3 2 1).")

  ;; Test with both lists empty
  (parachute:is equal (splice nil nil) nil
                 "Splicing two empty lists should yield an empty list.")
...
(define-test ensure-list-test
  ...
  (parachute:is equal (ensure-list '()) nil
                 "Passing an empty list should return it in a list.")
...
(define-test struct-field-names-test
  ...
  (let ((test-instance (make-test-struct :field1 1 :field2 2 :field3 3)))
    (parachute:is equal (struct-field-names test-instance) '("FIELD1" "FIELD2" "FIELD3")
                 "Field names for 'test-struct' should be 'FIELD1', 'FIELD2', and 'FIELD3'."))
  )
...
(defun proper-plist-p (plist)
  ...
  1. It contains an even number of elements, alternating between keys and values.
  2. All keys in the list are keywords.

  Parameters:
  - plist: A list to be checked. It is expected to be a property list where keys are potentially followed by corresponding values.

  Returns:
  T (true) if PLIST is a properly formed property list according to the above conditions. Otherwise, returns NIL (false).

  Example Usage:
  (proper-plist-p '(:key1 value1 :key2 value2)) ;; Returns T
  (proper-plist-p '(:key1 value1 key2 value2))  ;; Returns NIL (key2 is not a keyword)
  (proper-plist-p '(:key1 :key2 :key3))         ;; Returns NIL (odd number of elements)

  Notes:
  This function is useful for validating property lists before their use in situations where proper structure is required for correct operation. The function checks for the even length of the list and that each key is a valid keyword."
...
(defun drop (n sequence)
  "Removes the first N elements from SEQUENCE and returns the remaining elements.

  Parameters:
  - n: An integer representing the number of elements to be dropped from the beginning of SEQUENCE. 
       If N is greater than the length of SEQUENCE, an empty sequence is returned.
       If N is less than 0, the function behavior is implementation-dependent.
  - sequence: A sequence (list, vector, etc.) from which elements will be removed.

  Returns:
  A new sequence containing the elements of the original SEQUENCE, excluding the first N elements.
  ...
  Notes:
  - The function uses 'subseq', which creates a new sequence, leaving the original SEQUENCE unmodified.
  - If SEQUENCE is empty, the function returns an empty sequence regardless of the value of N.
  - This function is useful for operations where the initial part of a sequence is not needed."
  (subseq sequence n))


(defun read-file-as-bytes (filename)
  "Reads the contents of a file specified by FILENAME and returns it as a list of bytes.
  ...
  Example Usage:
  (read-file-as-bytes \"/path/to/file.txt\") ;; Returns the content of file.txt as a list of bytes

  Notes:
  This function is intended for reading binary files, as it treats the file content as a sequence of bytes.
  For text files, consider using character-based reading functions to handle different character encodings properly.
  The function uses 'with-open-file' to safely open and close the file stream."
  (with-open-file (stream filename :element-type '(unsigned-byte 8) :if-does-not-exist nil)
    (loop for byte = (read-byte stream nil) while byte collect byte)))


(defun create-mock-file (content)
  "Creates a temporary file in the system's temporary directory with the specified content.
  ...
  Side Effects:
  This function creates a new file on the file system. The file will be located in the '/tmp' directory and have a name based on the current universal time to ensure uniqueness.

  Example Usage:
  (create-mock-file #(72 101 108 108 111)) ;; Creates a file containing the bytes for 'Hello'

  Notes:
  The created file should be explicitly deleted when it is no longer needed to avoid leaving temporary files on the system.
  "

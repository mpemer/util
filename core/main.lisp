...
;; #.#~
;; "My docstring ~
;; is run through (format nil <the-doc-string>)
;; and as such, lines are concatenated by ~,
;; and otherwise front-trimmed and newlined,
;; because ~@ is appended lines with no ~.
...
(defun parse-number? (s)
  ...
  If S is non-nil and represents a valid number, the function parses ~
  S into a number and then rationalizes it, returning the rational ~
  representation of the number. If S is nil or does not represent a ~
  valid number, the function returns NIL."
  (handler-case
...
;;   (fail (parse-number? "abc") SB-INT:SIMPLE-PARSE-ERROR
...
(defun roundto (number decimals)
  ...
  DECIMALS, rounding the result to the nearest integer, and then ~
  dividing it back by the same factor.
...
(defun splice (add lst)
  #.#~ "Concatenates two lists by adding ADD in reverse order ~
  to the front of LST.
...
(defun ensure-list (item)
  ...
  - ITEM: An object to be transformed into a list ~
...
(defun struct-field-names (struct)
  ...
  The function uses the Common Lisp Object System (CLOS) Metaobject ~
  Protocol (MOP) to introspect the struct and retrieve the slot ~
  definitions.
  It then extracts and returns the names of these slots as a list of ~
  strings, which represent the field names.

  Each field name is converted from a symbol to a string to ensure ~
  compatibility with external systems and formats, such as CSV ~
  headers.
...
(defun struct-to-list (struct)
  ...
  Metaobject Protocol (MOP) to introspect the given struct and access ~
  each of its slots. It then retrieves the value of each slot, ~
  preserving the order in which the slots are defined.
...
(defun proper-plist-p (plist)
  ...
  Parameters:
  - plist: A list to be checked. It is expected to be a property list ~
           where keys are potentially followed by corresponding values.
...
(defun create-mock-file (content)
  ...
  Notes:
  The created file should be explicitly deleted when it is no longer ~
  needed to avoid leaving temporary files on the system."
